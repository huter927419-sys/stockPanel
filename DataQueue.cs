...
{
    /// <summary>
    /// 线程安全的队列包装类（.NET 3.5不支持ConcurrentQueue）
    /// </summary>
    ...
    public class DataQueue
    {
        // 线程安全的数据队列（使用优先级队列）
        // 实时数据优先处理，避免被分钟数据阻塞
        private ThreadSafeQueue<DataPacket> realTimeQueue = new ThreadSafeQueue<DataPacket>();  // 实时数据队列（高优先级）
        private ThreadSafeQueue<DataPacket> otherDataQueue = new ThreadSafeQueue<DataPacket>();  // 其他数据队列（低优先级，包含分钟数据、码表数据）
        // F10功能已禁用
        // private ThreadSafeQueue<DataPacket> f10DataQueue = new ThreadSafeQueue<DataPacket>();    // F10数据队列（独立线程，最低优先级）
...
#pragma warning restore CS0414
        ...
        // 为了兼容，保留原队列用于统计
        private ThreadSafeQueue<DataPacket> dataQueue = new ThreadSafeQueue<DataPacket>();
        
        // 用于通知处理线程的信号量
        private AutoResetEvent realTimeDataAvailableEvent = new AutoResetEvent(false);  // 实时数据信号
        private AutoResetEvent otherDataAvailableEvent = new AutoResetEvent(false);     // 其他数据信号
        // F10功能已禁用
        ...
        public void Enqueue(DataPacketType type, IntPtr lParam, int wParam)
        {
            ...
            // 根据数据类型选择队列
            ThreadSafeQueue<DataPacket> targetQueue;
            ...
            try
            {
                ...
                if (packet != null)
                {
                    ...
                    lock (statsLock)
                    {
                        ...
                        {
                            maxQueueSize = totalSize;
                        }
                    }
                    
                    // 记录入队成功（实时数据每10条记录一次，其他数据每100条）
                    if (type == DataPacketType.RealTimeData)
                    {
                        if (totalEnqueued % 10 == 0)
                        {
                            Logger.Instance.Debug(string.Format("实时数据包入队成功: 队列大小={0}, 累计入队={1}", realTimeQueue.Count, totalEnqueued));
                        }
                    }
                }
                else
        ...
        {
            Logger.Instance.Info("实时数据处理线程开始运行（优先级：Highest）");
            ...
            while (!shouldStop)
            {
                ...
                {
                    // 等待实时数据到达或停止信号
                    if (realTimeDataAvailableEvent.WaitOne(100)) // 最多等待100毫秒，快速响应
                    {
                        // 处理实时数据队列
                ...
                catch (Exception ex)
                {
                    ...
                    System.Threading.Thread.Sleep(10); // 出错后短暂休息，快速恢复
                }
            }
        ...
        private void ProcessOtherDataLoop()
        {
            ...
            while (!shouldStop)
            {
                ...
                {
                    // 等待其他数据到达或停止信号
                    if (otherDataAvailableEvent.WaitOne(1000)) // 最多等待1秒
                    {
                        // 处理其他数据队列
                        ProcessOtherQueue();
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    System.Threading.Thread.Sleep(100); // 出错后稍作休息
                }
            }
        ...
        private void ProcessRealTimeQueue()
        {
            ...
            const int MAX_BATCH_SIZE = 200; // 实时数据批次可以更大，因为处理速度快
            
            while (processedCount < MAX_BATCH_SIZE && !realTimeQueue.IsEmpty)
            {
                if (realTimeQueue.TryDequeue(out DataPacket packet))
                {
                    try
                    {
                        ...
                        // 记录处理成功（每100条记录一次，减少日志量）
                        if (totalProcessed % 100 == 0)
                        {
                            Logger.Instance.Debug(string.Format("实时数据包处理: 累计处理={0}, 队列剩余={1}", totalProcessed, realTimeQueue.Count));
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void ProcessOtherQueue()
        {
            ...
            while (processedCount < MAX_BATCH_SIZE && !otherDataQueue.IsEmpty)
            {
        ...
        private void ProcessPacket(DataPacket packet)
        {
            ...
            try
            {
                // 如果LParam不为空，说明在Enqueue时已经分配了内存，直接使用
                // 注意：当前实现中，Enqueue时不会分配非托管内存，LParam应该总是IntPtr.Zero
                // 但保留此逻辑以支持未来可能的扩展
                if (packet.LParam != IntPtr.Zero)
                {
                    dataPtr = packet.LParam;
                    needFree = true;  // 需要释放，因为是在Enqueue时分配的
                        ...
                        try
                        {
                            ...
                            if (header.m_nPacketNum == 0 && packet.DataBuffer != null && packet.DataBuffer.Length >= headerSize)
                            {
                                ...
                                try
                                {
                                    ...
                                    // 确保分配的内存足够大
                                    int requiredSize = Math.Max(packet.DataSize, headerSize + (int)header.m_nPacketNum * 158);
        ...
        public string GetStatistics()
        {
            lock (statsLock)
            {
                ...
                return string.Format("队列: 实时={0}, 其他={1}, 总计={2}, 已入队: {3}, 已处理: {4}, 最大队列: {5}", realTimeCount, otherCount, realTimeCount + otherCount, totalEnqueued, totalProcessed, maxQueueSize);
            }
        }
